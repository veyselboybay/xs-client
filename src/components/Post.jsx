...
const Post = ({ post }) => {
    ...
    return (
        <div className='post'>
            ...
            {showFullContent && <p className='content'>{post.content} {post.linkUrl && <><br /><a style={{ fontSize: 'small' }} href={linkUrl} target='_blank'>Go to that link</a></>} <span style={{ fontSize: 'smaller', color: 'blue', cursor: 'pointer' }} onClick={(e) => handleFullContent(e)}> ...less</span></p>}
            {!showFullContent && <p className='content'>{post.content.substring(0, 100)}{post.linkUrl && <><br /><a style={{ fontSize: 'small' }} href={linkUrl} target='_blank'>Go to that link</a></>}{post.content.length > 100 && <span style={{ fontSize: 'smaller', color: 'blue', cursor: 'pointer' }} onClick={(e) => handleFullContent(e)}> ...more</span>}</p>}
